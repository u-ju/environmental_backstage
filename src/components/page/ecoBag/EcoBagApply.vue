    ...
    <div class="table" v-loading="loading">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-goods"></i> 环保袋申请列表</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
                ...
                <el-table-column prop="order_status_name" label="审核状态"  align="center">
                	<template slot-scope="scope">
						<el-tag disable-transitions >{{scope.row.order_status_name}}</el-tag>
				  	</template>
                </el-table-column>
        ...
        <el-dialog title="编辑" :visible.sync="editVisible" width="40%">
            <el-form ref="form" :model="form" label-width="100px">
                ...
                <el-form-item label="审核">
                    <el-select v-model="form.status" :disabled="status==2?true:false"  placeholder="请选择">
                    	<el-option
					      v-for="item in sele"
					      :key="item.id"
					      :value="item.id"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page=1) {
            	var that =this;
				this.$axios.get(that.ports.ecoBag.applyIndex+"?page="+page).then(function(res) {
...
//				    that.user_status = res.data.result.user_status;
				    that.sele = res.data.result.eco_bag_order_status;
				    })
			  },
            search() {
                this.is_search = true;
            ...
            saveEdit() {
            	...
            	var data=that.qs.stringify({
            		...
            		order_status:that.form.status,
            		status_remark:that.form.status_remark,
	            })
            	that.$axios.post(that.ports.ecoBag.applyUpdate, data)
				.then(function (res) {
					console.log(res);
