    ...
    <div class="table" v-loading="loading">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-delete"></i> 垃圾分类</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button type="primary" icon="el-icon-circle-plus-outline" @click='handleEdit(-1)'>添加垃圾分类</el-button>
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
                ...
                <el-table-column prop="intro" label="价格简介"   align="center" max-width='300'>
                	<template slot-scope="scope">
        ...
        <el-dialog :title="idx==-1?'添加':'编辑'" :visible.sync="editVisible" width="40%">
            <el-form ref="form"  label-width="100px">
            	...
            	<!--<el-form-item label="分类id" v-if='idx!=-1'>
                    {{form.id}}
                </el-form-item>-->
...
//              	id:'',
        ...
        created() {
            this.getData();
            this.getSele();
            console.log(this.ports.garbageCate)
        },
        computed: {
            data() {
        ...
        methods: {
            ...
            getData(page=1) {
            	var that =this;
				this.$axios.get(that.ports.garbageCate.index+"?page="+page).then(function(res) {
            ...
            getSele(){
				...
				this.$axios.get(that.ports.Home.config).then(function(res) {
				    that.garbageCate_status = res.data.result.garbageCate_status;
				    that.garbageCate_parent_id = res.data.result.garbageCate_parent_id;
				    that.garbageCate_cate = res.data.result.garbageCate_cate;
				})
			  },
			   upLoad (file) {
...
//		      	console.log(file)
            ...
            handleEdit(index, row) {
                ...
                if(this.idx!=-1){
                	that.form=row
                	that.dialogImageUrl=row.icon
                	that.garbageCate_status_val=row.status
//              that.form={
//              	name:res.data.result.name,
//              	parent_id:res.data.result.parent_id,
//              	price:res.data.result.price,
//              	intro:res.data.result.intro,
//              	icon:res.data.result.icon,
//              }
//              	that.$axios.get(that.ports.garbageCate.show+'?id='+that.tableData[that.idx].id)
//					.then(function (res) {
//						if(res.data.status==200){
////							that.garbageCate = res.data.result;
//							that.form={
//			                	name:res.data.result.name,
//			                	parent_id:res.data.result.parent_id,
//			                	price:res.data.result.price,
...
//			                }
//							
//			                that.dialogImageUrl=res.data.result.icon,
////							console.log(that.form)
//							that.garbageCate_status_val=res.data.result.status
//						}else{
//							that.$message.warning(res.data.message);
//						}
//					})
...
//					});
            ...
            saveEdit() {
            	var that = this;
            	var data = this.form;
            	var url = that.ports.garbageCate.store;
            	if(that.idx!=-1){
            		data.cate_id=that.tableData[that.idx].id;
            		url = that.ports.garbageCate.update;
            	}
...
//              this.tableData.splice(this.idx, 1);
				...
				var data =that.qs.stringify({
            		cate_id:that.tableData[that.idx].id,
	            })
                that.$axios.post(that.ports.garbageCate.destroy, data)
                .then(function (res) {
                	if(res.data.status==200){
                		that.getData();
