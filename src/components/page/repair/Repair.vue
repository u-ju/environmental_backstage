    ...
    <div class="table" v-loading="loading">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-profile"></i> 维修中心</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                ...
                <el-select v-model="sel_cate" clearable  placeholder="维修分类" class="handle-select mr10">
                    <el-option
				      v-for="item in repair_cate"
				      :key="item.cate_id"
				      :value="item.cate_id"
				      :label="item.name">
				    </el-option>
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
                ...
                <!--<el-table-column  prop='cate_tag_name' label="类型名称" align="center">
                	<template slot-scope="scope">
						<template v-for="item in scope.row.cate_tag_name"><span>{{item}}</span></template>
				  	</template>
                </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page=1,url='') {
            	var that =this;
				this.$axios.get(that.ports.repair.index+"?page="+page+url).then(function(res) {
		    ...
		    getSele(){
				...
				this.$axios.get(that.ports.Home.config).then(function(res) {
				    ...
				    that.repair_cate = res.data.result.repair_cate;
				    that.repair_status = res.data.result.repair_status;
				})
			},
            search() {
                this.is_search = true;
            ...
            handleEdit(index, row) {
                ...
                this.$axios.get(that.ports.repair.show+'?id='+this.tableData[index].id).then(function(res) {
            ...
            saveEdit() {
            	...
            	console.log(data)
            	that.$axios.post(that.ports.repair.update, data)
				.then(function (res) {
					console.log(res);
...
//              this.tableData.splice(this.idx, 1);
                ...
                that.$axios.post(that.ports.repair.destroy, data)
                .then(function (res) {
                	if(res.data.status==200){
                		that.getData();
