    ...
    <div class="table" v-loading="loading">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-copy"></i> 邻里趣事</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button type="primary" icon="el-icon-circle-plus-outline" @click='handleEdit(-1)'>添加邻里趣事</el-button>
				<el-select v-model="content_source_val" clearable   placeholder="内容来源" class="handle-select mr10">
                    <el-option
				      v-for="item in vicinage_content_source"
				      :key="item.key"
				      :value="item.key"
				      :label="item.name">
				    </el-option>
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
            	<el-table-column prop="vicinage_id" label="ID" sortable width="100" align="center">
                </el-table-column>
                ...
                <el-table-column label="操作" width="220" align="center">
                    <template slot-scope="scope">
                    	...
                    	<router-link :to="{path:'/Reply',query: {id: scope.row.vicinage_id}}">
        ...
        <el-dialog :title="idx==-1?'添加':'编辑'" :visible.sync="editVisible" width="900px">
            <el-form ref="form"  label-width="100px">
                ...
                <el-form-item label="状态" v-if="idx!=-1">
                    <el-select v-model="vicinage_status_val"   placeholder="请选择">
                    	<el-option
					      v-for="item in vicinage_status"
					      :key="item.id"
					      :value="item.id"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page=1,url='') {
            	var that =this;
				this.$axios.get(that.ports.vicinage.index+"?page="+page+url).then(function(res) {
            ...
            getSele(){
				...
				this.$axios.get(that.ports.Home.config).then(function(res) {
				    that.vicinage_status = res.data.result.vicinage_status;
				    that.vicinage_content_source = res.data.result.vicinage_content_source;
				    that.vicinage_cate = res.data.result.vicinage_cate;
				})
			  },
			search() {
            ...
            handleEdit(index, row) {
                ...
                if(this.idx!=-1){
                	that.$axios.get(that.ports.vicinage.show+'?vicinage_id='+that.tableData[that.idx].vicinage_id)
					.then(function (res) {
						if(res.data.status==200){
//							that.vicinage = res.data.result;
							that.form={
							...
							that.vicinage_status_val=res.data.result.status
							that.setVisiblity(that.form.content)
						}else{
							that.$message.warning(res.data.message);
						}
            ...
            saveEdit() {
            	var that = this;
            	var data = this.form;
            	var url = that.ports.vicinage.store;
            	if(that.idx!=-1){
            		data.vicinage_id=that.tableData[that.idx].vicinage_id;
            		data.status = that.vicinage_status_val
            		url = that.ports.vicinage.update;
            	}
...
//              this.tableData.splice(this.idx, 1);
				...
				var data =that.qs.stringify({
            		vicinage_id:that.tableData[that.idx].vicinage_id,
	            })
                that.$axios.post(that.ports.vicinage.destroy, data)
                .then(function (res) {
                	if(res.data.status==200){
                		that.getData();
