            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
                ...
                <el-table-column label="操作" width="220" align="center">
                    <template slot-scope="scope">
                    	...
                    	<router-link :to="{path:'/buurtNReply',query: {id: scope.row.id}}">
        ...
        <el-dialog :title="idx==-1?'添加':'编辑'" :visible.sync="editVisible" width="1200px">
            <el-form ref="form"  label-width="100px">
                ...
                <el-form-item label="状态" v-if="idx!=-1">
                    <el-select v-model="buurt_status_val"   placeholder="请选择">
                    	<el-option
					      v-for="item in buurt_status"
					      :key="item.id"
					      :value="item.id"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page=1,url='') {
            	var that =this;
				this.$axios.get(that.ports.buurtN.index+"?page="+page,{params:this.seach}).then(function(res) {
            ...
            handleEdit(index, row) {
                ...
                if(this.idx!=-1){
                	that.$axios.get(that.ports.buurtN.show+'?id='+that.tableData[that.idx].id)
					.then(function (res) {
...
//              this.tableData.splice(this.idx, 1);
                ...
                that.$axios.post(that.ports.buurtN.destroy, data)
                .then(function (res) {
                	if(res.data.status==200){
                		that.getData();
