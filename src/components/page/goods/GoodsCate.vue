<template>
	...
	<div class="crumbs">
	    <el-breadcrumb separator="/">
	        <el-breadcrumb-item><i class="el-icon-lx-tag"></i> 商品分类</el-breadcrumb-item>
	    </el-breadcrumb>
	</div>
	<el-row class='pading'>
...
//      console.log(data);
       ...
       this.$axios.get(that.ports.goodsc.index).then(function(res) {
        	var tshopCate= res.data.result.list;
        	that.tshopCate =[{id: 0,name: '商品分类',parent_id:0, children: tshopCate}]
        	that.loading=false
       })
      },
      getSele(){
...
//      this.data_shopCate=data;
      ...
      saveEdit(){
      	var that = this;
    	var url = that.ports.goodsc.store;
    	var data=this.form;
    	if(that.id!=-1){
    		data.cate_id = that.id;
    		url = that.ports.goodsc.update;
    	}
		...
		.then(function (res) {
			if(res.data.status==200){
				if(that.id==-1){
					const newChild = { id: res.data.result.cate_id,type: that.form.type,name: that.form.name,parent_id:that.form.parent_id, children: [] };
      ...
      deleteRow(){
        ...
        that.$axios.post(that.ports.goodsc.destroy, data)
        .then(function (res) {
