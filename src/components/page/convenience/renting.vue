    ...
    <div class="table" v-loading="loading">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-shop"></i> 房屋出租</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
				...
				<el-select v-model="search.decoration" clearable  placeholder="审核状态" class="handle-select mr10">
                    <el-option
				      v-for="item in decoration1"
				      :key="item.id"
				      :value="item.id"
				      :label="item.name">
				    </el-option>
                </el-select>
                <el-select v-model="search.room" clearable  placeholder="经验" class="handle-select mr10">
                    <el-option
				      v-for="item in room1"
				      :key="item.id"
				      :value="item.id"
                ...
                <el-select v-model="search.mode" clearable  placeholder="审核状态" class="handle-select mr10">
                    <el-option
				      v-for="item in mode1"
				      :key="item.id"
				      :value="item.id"
                ...
                <el-select v-model="search.rent" clearable  placeholder="经验" class="handle-select mr10">
                    <el-option
				      v-for="item in rent1"
				      :key="item.id"
				      :value="item.id"
                ...
                <el-input class='width200' v-model="search.keywords" placeholder="请输入用户ID"></el-input>
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" id="out-table">
                ...
                <el-table-column  prop='room_name' label="房屋类型" align="center" >
                </el-table-column>
                <el-table-column  prop='mode_name' label="出租方式" align="center" >
                </el-table-column>
        ...
        <el-dialog title="编辑" :visible.sync="editVisible" width="40%">
            <el-form ref="form" :model="form" label-width="100px">
                ...
                <el-form-item label="图片	">
                    <viewer   :images="result.images">
                    	<template v-for="item in result.images">
                    		<img  class='margin_pic' :src="item"  width="320">
                    	</template>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page=1,url='') {
				...
				this.$axios.get(that.ports.renting.index+"?page="+page,{params:data}).then(function(res) {
			...
			getsele() {
            	var that =this;
				this.$axios.get(that.ports.renting.conf).then(function(res) {
			        that.sele = res.data.result.status;
			        that.room1 = res.data.result.room;
			        that.decoration1 = res.data.result.decoration;
			        that.mode1 = res.data.result.mode;
			        that.rent1 = res.data.result.price_interval;
			    })
            },
            searchbtn() {
            ...
            handleEdit(index, row) {
                ...
                this.$axios.get(that.ports.renting.show+this.idx).then(function(res) {
            ...
            saveEdit() {
            	...
            	that.$axios.post(that.ports.renting.update, data)
				.then(function (res) {
					console.log(res);
            ...
            deleteRow(){
                ...
                that.$axios.post(that.ports.renting.destroy, data)
                .then(function (res) {
                	if(res.data.status==200){
                		that.getData();
