<template>
	...
	<div class="crumbs">
	    <el-breadcrumb separator="/">
	        <el-breadcrumb-item><i class="el-icon-lx-tag"></i> 商店分类</el-breadcrumb-item>
	    </el-breadcrumb>
	</div>
	<el-row class='pading'>
...
//      console.log(data);
       ...
       getData(){
    	var that =this;
		this.$axios.get(that.ports.shopCate.index).then(function(res) {
        	var shopCate= res.data.result.list;
        	that.shopCate =[{cate_id: 0,name: '商店分类',parent_id:0, children: shopCate}]
       }) 
      },
      getSele(){
    	var that =this;
      ...
      getshopCate(cate_id=0){
    	...
    	var that =this;
		this.$axios.get(that.ports.shopCate.show+"?cate_id="+cate_id).then(function(res) {
      ...
      saveEdit(){
		...
		.then(function (res) {
			if(res.data.status==200){
				if(that.cate_id==-1){
					const newChild = { cate_id: res.data.result.cate_id,type: that.form.type,name: that.form.name,parent_id:that.form.parent_id, children: [] };
			        ...
			        const children = parent.data.children || parent.data;
			        const index = children.findIndex(d => d.cate_id === that.data_shopCate.cate_id);
