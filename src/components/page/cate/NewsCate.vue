...
//      console.log(data);
       ...
       getData(){
		...
		this.$axios.get(that.ports.newsCate.index).then(function(res) {
        	...
        	that.newsCate =[{cate_id: 0,name: '新闻分类',parent_id:0, children: newsCate}]
       }) 
      },
      getnewsCate(cate_id=0){
      	if(cate_id==0){
    	...
    	var that =this;
		this.$axios.get(that.ports.newsCate.show+"?cate_id="+cate_id).then(function(res) {
		...
		.then(function (res) {
			if(res.data.status==200){
				if(that.cate_id==-1){
			        ...
			        const children = parent.data.children || parent.data;
			        const index = children.findIndex(d => d.cate_id === that.data_newsCate.cate_id);
