<template>
	...
	<div class="crumbs">
	    <el-breadcrumb separator="/">
	        <el-breadcrumb-item><i class="el-icon-lx-tag"></i> 分类指南</el-breadcrumb-item>
	    </el-breadcrumb>
	</div>
	<el-row class='pading'>
...
//      console.log(data);
       ...
       getData(){
    	var that =this;
//		this.$axios.get(that.ports.guide.index+"?type=1").then(function(res) {
//      	var guide= res.data.result.list;
//      	that.guide =[{id: 0,name: '便民商家',parent_id:0, children: guide}]
//     }) 
       this.$axios.get(that.ports.guide.index).then(function(res) {
        	var tguide= res.data.result.list;
        	that.tguide =[{id: 0,name: '分类指南',parent_id:0, children: tguide}]
        	that.loading=false
       })
      },
      getSele(){
      ...
      getguide(id=0){
    	...
    	var that =this;
//		this.$axios.get(that.ports.guide.show+"?id="+id).then(function(res) {
...
//		          that.dialogVisible = true;
      ...
      append(data,num) {
      	...
      	if(num==1){
      		this.data_guide = data;
      		this.form.type = 1;
      	}else{
      		this.data_tguide = data;
      		this.form.type = 2;
	  ...
	  edit(node, data,num) {
        ...
        if(num==1){
      		this.data_guide = data;
      		this.node_guide=node;
      	}else{
      		this.data_tguide = data;
      		this.node_tguide=node;
      	}
        this.id = data.id;
...
//      this.data_guide=data;
      ...
      saveEdit(){
      	var that = this;
    	var url = that.ports.guide.store;
    	var data=this.form;
    	if(that.id!=-1){
    		data.cate_id = that.id;
    		url = that.ports.guide.update;
    	}
		...
		.then(function (res) {
			if(res.data.status==200){
				if(that.id==-1){
			        ...
			        if(that.num==1){
			        	if (!that.data_guide.children) {
				          that.$set(that.data_guide, 'children', []);
				        }
				        that.data_guide.children.push(newChild);
			        }else{
			        	if (!that.data_tguide.children) {
				          that.$set(that.data_tguide, 'children', []);
				        }
				        that.data_tguide.children.push(newChild);
			        }
					...
					if(that.num==1){
						const parent = that.node_guide.parent;
						const children = parent.data.children || parent.data;
						const index = children.findIndex(d => d.id === that.data_guide.id);
						children[index].name=that.form.name
				        children[index].type=that.form.type
						that.$message.success(`修改成功`);
					}else{
						
						const parent = that.node_tguide.parent;
						const children = parent.data.children || parent.data;
						const index = children.findIndex(d => d.id === that.data_tguide.id);
						console.log(parent)
      ...
      deleteRow(){
        ...
        that.$axios.post(that.ports.guide.destroy, data)
        .then(function (res) {
