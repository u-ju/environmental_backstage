<template>
    <div class="login-wrap">
        <div class="ms-login">
            <div class="ms-title color409EFF">啄木鸟环保后台管理系统</div>
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="ms-content" v-show="!show">
                <el-form-item prop="username">
                    <el-input v-model.number="ruleForm.username" placeholder="手机号" maxlength='11'>
                        <el-button slot="prepend" icon="el-icon-lx-people"></el-button>
                    </el-input>
                </el-form-item>
                <el-form-item prop="password">
                    <el-input  placeholder="验证码" class='password' v-model.number="ruleForm.password" @keyup.enter.native="submitForm('ruleForm')"  maxlength='6'>
            ...
            <el-form :model="ruleForm1"  :rules="rules1" ref="ruleForm1" l label-width="0px" class="ms-content" v-show="show">
                <el-form-item prop="username">
                    <el-input v-model.number="ruleForm1.username" placeholder="用户名/手机号/邮箱" >
                        <el-button slot="prepend" icon="el-icon-lx-people"></el-button>
                    </el-input>
                </el-form-item>
                <el-form-item prop="password">
                	<el-input type="password" v-model.number="ruleForm1.password" placeholder="密码" >
                        <el-button slot="prepend" icon="el-icon-lx-lock"></el-button>
                   </el-input>
                </el-form-item>
                <div class="login-btn">
...
//                  	console.log(valid)
                    	that.$axios.post(that.ports.Login.mobileLogin,data).then(function(res) {
                    		...
                    		if(res.data.status==200){
								...
								that.$axios({
									method:"get",
									url:that.ports.user.authShow,
									headers:{
										'token':res.data.result.token
									}
								}).then(function(res1) {
				            		if(res1.data.status==200){
				            			localStorage.setItem('wyhb_username',res1.data.result.nickname);
				            			localStorage.setItem('wyhb_avatar',res1.data.result.avatar);
//				            			localStorage.setItem('login','wyhb');
										store.commit('set_login', 'wyhb');
